...
app.post("/search/resumes", async (req, res) => {
  ...
  try {
    ...
    const normalizedSearchType = parsed.searchType === "bm25" ? "keyword" : parsed.searchType;
    ...
    // Ensure each result has an id so frontend can call /candidate/:id.
    if (mongoClient) {
      const missingIdResults = results.filter((result) => !result.id && result.email && result.email !== "Not found");
      if (missingIdResults.length > 0) {
        const emails = Array.from(new Set(missingIdResults.map((result) => result.email)));
        const collection = mongoClient.db(config.mongodb.dbName).collection(config.mongodb.collection);
        ...
        const idByEmail = new Map<string, string>(
          candidates
            .filter((candidate) => typeof candidate.email === "string")
            .map((candidate) => [String(candidate.email), candidate._id.toString()])
        );

        results.forEach((result) => {
          if (!result.id && idByEmail.has(result.email)) {
            result.id = idByEmail.get(result.email);
          }
        });
      }
    }
...
// Candidate detail endpoint by MongoDB _id
app.get("/candidate/:id", async (req, res) => {
  const requestId = `candidate_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

  try {
    const id = req.params.id;
    console.log(`\n[${requestId}] === CANDIDATE DETAIL REQUEST ===`);
    console.log(`[${requestId}] Candidate ID: ${id}`);
    ...
    const skills: string[] = Array.isArray(doc.skills)
      ? doc.skills.map((skill: unknown) => String(skill))
      : typeof doc.skills === "string"
        ? doc.skills.split(",").map((skill: string) => skill.trim()).filter(Boolean)
        : [];
    ...
    const candidate = {
      ...
      skills,
      resumeContent: doc.text || "",
      experience: doc.total_Experience ? `${doc.total_Experience} years` : undefined,
      specialization: doc.role || undefined,
      location: doc.location || undefined,
      linkedinProfile: undefined,
      education: doc.education ? [doc.education] : undefined,
      certifications: undefined,
    };

    console.log(`[${requestId}] Candidate found: ${candidate.name}`);
    console.log(`====================================\n`);

    // Return in CandidateProfile format expected by frontend
    res.json({
      data: {
        candidate
      }
    ...
    console.error(`\n[${requestId}] === CANDIDATE DETAIL ERROR ===`);
    console.error(`Error:`, err.message ?? String(err));
    console.error(`Stack:`, err.stack);
    console.error(`====================================\n`);
...
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    if (shouldUseFilter) {
      ...
      console.log(`[${requestId}] Search type: ${parsed.searchType}, Top K: ${parsed.topK}`);
      if (parsed.hybridVectorWeight !== undefined) {
        console.log(`[${requestId}] Hybrid vector weight: ${parsed.hybridVectorWeight}%`);
      }
