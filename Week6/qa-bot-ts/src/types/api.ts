...
export const ConversationalQuerySchema = z.object({
  ...
  searchType: z.enum(["keyword", "vector", "hybrid"]).optional().default("hybrid"),
  hybridVectorWeight: z.number().min(0).max(100).optional().default(50), // 0-100 scale for vector weight in hybrid search
});
