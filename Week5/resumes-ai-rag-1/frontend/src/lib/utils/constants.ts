...
export const SEARCH_MODE_COLORS: Record<SearchMode, string> = {
...
export const SCORE_FORMATTER = (score: number, precision = 2): string => {
    return score.toFixed(precision);
};

export const DURATION_FORMATTER = (ms: number): string => {
    if (ms < 1000) return `${Math.round(ms)}ms`;
    return `${(ms / 1000).toFixed(1)}s`;
};

export const EXPERIENCE_FORMATTER = (years: number | undefined): string => {
    if (!years) return 'N/A';
...
export const getScoreLabel = (mode: SearchMode): string => {
    return mode === 'vector'
        ? 'Similarity'
        : mode === 'bm25'
            ? 'Keyword Score'
...
export const getScoreColor = (mode: SearchMode): string => {
