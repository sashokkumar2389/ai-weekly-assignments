...
export function useSearch() {
    ...
    const submitQuery = useCallback(async (query) => {
        ...
        try {
            ...
            const totalApiTime = data.componentTimings.bm25Ms +
                data.componentTimings.vectorMs +
                data.componentTimings.rerankMs;
            setResults(data.results, data.query, totalApiTime, data.counts.bm25, data.counts.vector, data.counts.merged, data.rerankUsed);
