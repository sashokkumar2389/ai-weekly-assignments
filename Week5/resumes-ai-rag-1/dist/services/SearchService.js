...
class SearchService {
    constructor() {
        this.resumeRepository = new ResumeRepository_1.ResumeRepository();
    ...
    async endToEndSearch(query, filters, options) {
        ...
        try {
            ...
            const rerankResponse = await this.llmService.rerankCandidates(query, candidatesForReranking, options.topK || 10);
            const rerankMs = Date.now() - rerankStart;
            componentTimings.rerankMs = rerankMs;
            let results = (rerankResponse.rankedCandidates || []).map((candidate) => {
                const resumeId = candidate.resumeId || '';
                const originalResume = combinedResults.find(r => (r._id || r.resumeId || '') === resumeId);
                return {
                    ...candidate,
                    _id: resumeId,
                    resumeId,
            ...
            if (options.summarize) {
                ...
                results = await Promise.all(results.map(async (candidate) => {
                    try {
                        const summary = await this.llmService.summarizeCandidateFit(query, {
     ...
     */
    combineAndDeduplicateWithSources(bm25Results, vectorResults) {
        ...
        for (const result of bm25Results) {
            ...
            if (id) {
                ...
                if (!sourcesMap[id])
                    sourcesMap[id] = { bm25: null, vector: null };
